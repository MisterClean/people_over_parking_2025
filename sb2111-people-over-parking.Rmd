...
download_and_extract_gtfs <- function(agency_name, zip_link) {
  ...
  force_download <- identical(Sys.getenv("FORCE_GTFS_DOWNLOAD"), "1")
...
# Apply conversion to all datasets and preserve modifications
gtfs_datasets <- list(
  cta = cta_data,
  pace = pace_data,
  metra = metra_data,
...
gtfs_datasets <- lapply(gtfs_datasets, function(data) {
  ...
  if ("stop_sequence" %in% names(data$stop_times)) {
    data$stop_times[, stop_sequence := as.integer(stop_sequence)]
  }
  data$calendar <- convert_integer64(data$calendar)
  data
})

cta_data <- gtfs_datasets$cta
pace_data <- gtfs_datasets$pace
metra_data <- gtfs_datasets$metra
metro_stl_data <- gtfs_datasets$metro_stl
cumtd_data <- gtfs_datasets$cumtd
...
if ("stop_sequence" %in% names(all_stop_times)) {
  all_stop_times[, stop_sequence := as.integer(stop_sequence)]
}
...
# Load Illinois boundary and helper clipping utilities
illinois_boundary <- states(cb = TRUE, year = 2023) %>%
  dplyr::filter(STUSPS == "IL") %>%
  st_transform(4326)
illinois_boundary_geom <- st_geometry(illinois_boundary)

clip_sfc_to_il <- function(geom) {
  if (is.null(geom) || length(geom) == 0) {
    return(geom)
  }
  if (all(st_is_empty(geom))) {
    return(geom)
  }
  st_intersection(geom, illinois_boundary_geom)
}

clip_sf_to_il <- function(sf_obj) {
  if (is.null(sf_obj) || nrow(sf_obj) == 0) {
    return(sf_obj)
  }
  clipped_geom <- clip_sfc_to_il(st_geometry(sf_obj))
  st_geometry(sf_obj) <- clipped_geom
  sf_obj <- sf_obj[!st_is_empty(sf_obj), ]
  sf_obj
}

# Restrict analysis to stops within Illinois
all_stops_sf <- st_as_sf(all_stops, coords = c("stop_lon", "stop_lat"), crs = 4326, remove = FALSE)
within_il <- lengths(st_within(all_stops_sf, illinois_boundary_geom)) > 0
all_stops_sf <- all_stops_sf[within_il, ]
all_stops <- as.data.table(st_drop_geometry(all_stops_sf))

illinois_stop_ids <- unique(all_stops$unique_stop_id)
all_stop_times <- all_stop_times[unique_stop_id %in% illinois_stop_ids]

illinois_trip_ids <- unique(all_stop_times$unique_trip_id)
all_trips <- all_trips[unique_trip_id %in% illinois_trip_ids]

illinois_route_ids <- unique(all_trips$unique_route_id)
all_routes <- all_routes[unique_route_id %in% illinois_route_ids]

illinois_service_ids <- unique(all_trips$service_id)
if ("service_id" %in% names(all_calendar)) {
  all_calendar <- all_calendar[service_id %in% illinois_service_ids]
}
...
all_corridors_union_wgs84 <- clip_sfc_to_il(all_corridors_union_wgs84)

# Hubs
all_hub_areas_wgs84 <- st_transform(all_hub_areas, 4326)
all_hub_areas_wgs84 <- clip_sfc_to_il(all_hub_areas_wgs84)
half_mile_buffers_wgs84 <- st_transform(half_mile_buffers, 4326)
half_mile_buffers_wgs84 <- clip_sf_to_il(half_mile_buffers_wgs84)
...
metro_stl_hubs_union <- if(nrow(metro_stl_hub_buffers) > 0) st_union(metro_stl_hub_buffers) else st_sfc(crs = 4326)
cumtd_hubs_union <- if(nrow(cumtd_hub_buffers) > 0) st_union(cumtd_hub_buffers) else st_sfc(crs = 4326)
cta_hubs_union <- clip_sfc_to_il(cta_hubs_union)
pace_hubs_union <- clip_sfc_to_il(pace_hubs_union)
metra_hubs_union <- clip_sfc_to_il(metra_hubs_union)
metro_stl_hubs_union <- clip_sfc_to_il(metro_stl_hubs_union)
cumtd_hubs_union <- clip_sfc_to_il(cumtd_hubs_union)
...
all_affected_areas_combined <- clip_sfc_to_il(all_affected_areas_combined)
```
...
il_bbox <- st_bbox(illinois_boundary)

# Create the interactive map
map <- leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  fitBounds(il_bbox["xmin"], il_bbox["ymin"], il_bbox["xmax"], il_bbox["ymax"]) %>%
...
- St. Louis area: Metro St. Louis GTFS data (includes MetroLink and MetroBus)
