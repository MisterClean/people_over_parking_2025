...
```{r download_gtfs}
# Fixed direct URL for the GTFS download
...
cache_file <- file.path(cache_dir, "cta_gtfs.zip")
...
# Now we need to identify bus stops that meet the criteria of 2+ major routes with frequency ≤ 15 min
...
map <- leaflet() %>%
  ...
  addLegend(
    ...
    colors = c("purple"),
    labels = c("CTA Transit-served Areas Affected by the People Over Parking Act"),
    opacity = 0.7
  ) %>%
...
- [Housing costs in Illinois are rising. Lawmakers are considering several bills that could help](https://abundanthousingillinois.org/op-eds/housing-costs-in-illinois-are-rising-lawmakers-are-considering-several-bills-that-could-help/)
